<?php

namespace HeavenProject\GitBranchPanel;

use Tracy;

/**
 * Displays current Git branch on Tracy debug panel.
 */
class Panel implements Tracy\IBarPanel
{
    /** @var string */
    const IMAGE = '<img alt="" src="data:image/png;base64,"/>&nbsp;';

    /** @var string */
    private $entryPath;

    /** @var string */
    private $targetDir;

    /**
     * @param string $entryPath
     * @param string $targetDir
     */
    public function __construct($entryPath, $targetDir)
    {
        $this->entryPath = $entryPath;
        $this->targetDir = $targetDir;
    }

    /**
     * @inheritdoc
     */
    public function getTab()
    {
        return self::IMAGE . $this->getCurrentBranchName();
    }

    /**
     * @inheritdoc
     */
    public function getPanel()
    {
        return '';
    }

    /**
     * @return string
     */
    private function getCurrentBranchName()
    {
        $branch = new GitBranch($this->entryPath, $this->targetDir);

        return $branch->getBranchName();
    }
}
